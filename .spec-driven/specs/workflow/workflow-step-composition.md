...
The system MUST terminate the workflow as FAILED when any step fails, without executing subsequent steps.

#### Scenario: Step failure stops execution
- GIVEN a workflow with two steps where step A fails with reason `"connection refused"`
- WHEN step A fails
