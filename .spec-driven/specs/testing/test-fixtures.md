...
- GIVEN a `CapturingEventBus` that has received multiple events of different types
- WHEN `eventsOfType(EventType)` is called
- THEN only events matching the given type are returned
