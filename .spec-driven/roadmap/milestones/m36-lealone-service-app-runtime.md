...
- `service-runtime-packaging` - Declared: complete - 定义应用运行时的最小打包与启动约定，减少部署时对底层 Lealone 初始化细节的手工拼装
- `service-schema-bootstrap-governance` - Declared: complete - 为启动期建表、建服务和运行配置装配增加最小治理规则，限制不安全或不兼容的声明式变更进入自动启动链路
...
- M25 生产环境一键安装与修复可复用本里程碑产出的运行时与打包约定，但不构成本里程碑的前置依赖
