...
- `loop-recommend-auto-pipeline` - Declared: complete - Recommend → Auto 执行流水线：串联调用 roadmap-recommend 和 spec-driven-auto skill，处理每轮结果与错误分类
- `loop-progress-persistence` - Declared: complete - 循环进度持久化：记录执行历史、已完成的 change 链、失败/跳过原因、崩溃恢复点与审计日志

## Dependencies

- M04 Agent 生命周期与编排（循环本身作为 agent 运行，依赖状态机和会话管理）
- M07 任务与团队注册表（循环任务注册与管理）
