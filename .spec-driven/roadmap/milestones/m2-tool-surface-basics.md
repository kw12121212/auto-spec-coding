...
- `tool-bash` - Declared: planned - Bash 命令执行工具实现（ProcessBuilder + 超时控制）
- `tool-file-ops` - Declared: planned - 文件读/写/编辑工具实现（java.nio.file）
...
- tool-grep 优先使用 Java NIO 实现基础搜索；如需 ripgrep 高性能，由 M3 的工具管理器提供二进制
