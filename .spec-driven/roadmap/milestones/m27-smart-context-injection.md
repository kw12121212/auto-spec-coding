...
参考 Lealone `Service.genJavaCode()` 的反射筛选策略，在 LLM 调用前对 Conversation 历史和 ToolResult 做智能相关性裁剪与语义压缩，降低 Token 消耗、提升 LLM 响应质量。替代当前 `ContextWindowManager` 的简单截断策略。
...
- 发送给 LLM 的 ToolResult MUST 只包含当前 turn 引用的工具输出，无关工具输出被裁剪
...
- 有对比基准测试证明 Token 减少量（目标 ≥30%）和响应质量不下降

## Planned Changes

- `context-relevance-scorer` - Declared: planned - 定义 ContextRelevanceScorer 接口与默认关键词匹配实现，提供 ToolResult→turn 的相关性评分能力
- `tool-result-filter` - Declared: planned - 实现 ToolResultFilter 装饰器：在构建 LlmRequest 时根据当前 turn 的 tool call 列表过滤无关 ToolResult
- `conversation-summarizer` - Declared: planned - 实现 ConversationSummarizer：基于滑动窗口对早期对话历史生成语义摘要，替换简单截断策略
- `smart-context-injector` - Declared: planned - 整合上述组件为 SmartContextInjector 装饰器，透明包装 LlmClient，与 DefaultOrchestrator 和 LoopDriver 集成
