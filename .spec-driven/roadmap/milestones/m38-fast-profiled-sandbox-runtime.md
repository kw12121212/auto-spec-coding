...
- `profile-tool-execution-binding` - Declared: complete - 将 BashTool、后台进程和 loop phase 命令执行接入 profile 选择规则，保证项目命令在预期隔离环境中运行
- `profile-governance-observability` - Declared: complete - 增加 profile 使用的权限边界、审计事件、健康诊断和不支持环境的快速失败输出
