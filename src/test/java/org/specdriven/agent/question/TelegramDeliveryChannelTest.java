...
class TelegramDeliveryChannelTest {
    ...
    @Test
    void apiErrorThrowsMobileAdapterException() {
        startServer(403, "{\"ok\":false,\"description\":\"Forbidden\"}");

        String baseUrl = "http://localhost:" + server.getAddress().getPort() + "/bot";
        TelegramDeliveryChannel channel = new TelegramDeliveryChannel(
                baseUrl, "badToken", "chat123",
                HttpClient.newHttpClient(), messageMap);
    ...
    void connectionErrorThrowsMobileAdapterException() {
        String baseUrl = "http://localhost:1/bot";
        TelegramDeliveryChannel channel = new TelegramDeliveryChannel(
                baseUrl, "token", "chat123",
                HttpClient.newHttpClient(), messageMap);
    ...
    void closeDoesNotThrow() {
        TelegramDeliveryChannel channel = new TelegramDeliveryChannel(
                "http://localhost:1/bot", "token", "chat123",
                HttpClient.newHttpClient(), messageMap);
